...
:find
for /f "tokens=* delims=" %%a in ('where sdelete64.exe 2^>nul') do set "sdelete=%%a"
if exist "%sdelete%" (
    ...
    reg add "HKCU\Software\Classes\*\shell\wincmd64_sdelete\command" /ve /d "cmd.exe /c \"\"%sdelete%\" -nobanner \"%%1\" ^& pause\"" /f
